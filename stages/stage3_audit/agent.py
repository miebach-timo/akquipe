...
class AuditAgent:
    ...
    def run(self, data: ScrapedData, system_prompt: str, user_prompt: str) -> dict:
        messages = [{"role": "user", "content": user_prompt}]

        for iteration in range(10):  # max 10 Iterationen
...
async def run_audit(state: RunState, settings) -> RunState:
    ...
    try:
        ...
        result = agent.run(data, system_prompt, user_prompt)
